...
> The missing scipy + quantum-information toolkit for Apple Silicon.
> GPU-accelerated via [MLX](https://github.com/ml-explore/mlx).
...
exponential, STFT, Fisher information, quantum relative entropy, and a
statevector circuit simulator — under one consistent namespace
(`mlx_sci.special`, `mlx_sci.linalg`, `mlx_sci.signal`,
`mlx_sci.quantum`). Each sub-package is independently installable, so
...
`mlx-sci` is at **v0.2.0**. The 0.x series is still iterating — minor
